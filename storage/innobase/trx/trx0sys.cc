...
static void set_cluster_sched() {
  ...
  trx_sys->cluster_sched.resize(arr.size());
...
static void set_sched_deps_and_counts() {
  ...
  for (uint32_t i = 0; i < trx_sys->cluster_deps_arr.size(); i++) {
    if (i < trx_sys->cluster_deps_arr.size() / 2) {
      trx_sys->cluster_deps_arr[i][0] = 251;
    } else {
      trx_sys->cluster_deps_arr[i][0] = 1;
    }
  }
}
...
void trx_sys_close(void) {
  ...
  trx_sys->sched_counts.~vector<ut::unique_ptr<std::atomic_int>>();
